...
function onCreate() {
	try {
		...
		// If the dummy is missing, DON'T attempt the warm-up: playing a failed-to-load
		// VideoSprite poisons the VLC pipeline and breaks the next real startVideo.
		...
		trace('VideoWarmUp: warm-up skipped (' + e + ')');
	}
}
